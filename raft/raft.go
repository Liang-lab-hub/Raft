...
func (r *Raft) applyCommand(req *pb.ApplyCommandRequest) (*pb.ApplyCommandResponse, error) {
	...
	localLastId, _ := r.getLastLog()
	newEntry := make([]*pb.Entry, 0)
	entry :=  &pb.Entry{
		Id: localLastId+1,
		Term: r.currentTerm,
		Data: req.GetData(),
	...
	r.appendLogs(newEntry)
	
	// TODO: (B.1)* - return the new log entry
	resp := &pb.ApplyCommandResponse{
		Entry: entry,
	}
...
func (r *Raft) appendEntries(req *pb.AppendEntriesRequest) (*pb.AppendEntriesResponse, error) {
	...
	if prevLogId != 0 && prevLogTerm != 0 {
		...
		//fmt.Print("=============== CurrentId: ", r.id, "  , LastLogId: ", lastLogId, "  , LastLogTerm: ", lastLogTerm, "  , PrevLogId: ", prevLogId, ", PreLogTerm: ", prevLogTerm , "   ==================\n")
		if lastLogId < prevLogId {
			r.logger.Info("the given previous log from leader is missing or mismatched", zap.Uint64("prevLogId", prevLogId), zap.Uint64("prevLogTerm", prevLogTerm), zap.Uint64("logTerm", lastLogTerm))
			return &pb.AppendEntriesResponse{Term: r.currentTerm, Success: false}, nil
		} else {
	...
	if len(req.GetEntries()) != 0 {
		...
		//if uint64(len(r.logs)) >= prevLogId+1 {
		    //log := r.getLog(prevLogId+1)
			//if log.GetTerm() != req.GetTerm() {r.deleteLogs(prevLogId)}
	...
	if leaderCommitId  > r.commitIndex {
		if leaderCommitId  > lastLogId {r.setCommitIndex(lastLogId)} else {r.setCommitIndex(leaderCommitId)}
...
func (r *Raft) requestVote(req *pb.RequestVoteRequest) (*pb.RequestVoteResponse, error) {
	...
	// ----------
	// TODO: (A.7)
	if r.votedFor != 0 && r.votedFor != r.id {
		r.logger.Info("reject since already vote for another candidate", zap.Uint64("term", r.currentTerm), zap.Uint32("votedFor", r.votedFor))
		return &pb.RequestVoteResponse{Term: r.currentTerm, VoteGranted: false}, nil
	}
	/*
	if false {
	...
	local_last_id, local_last_term := r.getLastLog()
	if local_last_id > req.GetLastLogId() || local_last_term > req.GetLastLogTerm() {
...
func (r *Raft) broadcastAppendEntries(ctx context.Context, appendEntriesResultCh chan *appendEntriesResult) {
	...
	for peerId, peer := range r.peers {
		...
		if r.nextIndex[peerId]-1 >= 1 {
			req := &pb.AppendEntriesRequest{
				Term: r.currentTerm,
				LeaderId: r.id,
				LeaderCommitId: r.commitIndex,
				PrevLogId: r.getLog(r.nextIndex[peerId]-1).GetId(),
				PrevLogTerm: r.getLog(r.nextIndex[peerId]-1).GetTerm(),
...
func (r *Raft) handleAppendEntriesResult(result *appendEntriesResult) {
	...
	if result.req.GetTerm() > r.currentTerm || result.GetTerm() > r.currentTerm {
	...
	if !result.GetSuccess() {
		...
		if r.nextIndex[result.peerId]-1 < 1 {
			r.setNextAndMatchIndex(result.peerId, 1, 0)
		} else {
			r.setNextAndMatchIndex(result.peerId, r.nextIndex[result.peerId]-1, r.matchIndex[result.peerId])
		}
		
		r.logger.Info("append entries failed, decrease next index", zap.Uint64("nextIndex", r.nextIndex[result.peerId]), zap.Uint64("matchIndex", r.matchIndex[result.peerId]))
		...
		r.setNextAndMatchIndex(result.peerId, r.nextIndex[result.peerId]+uint64(len(entries)), r.nextIndex[result.peerId]+uint64(len(entries)-1))
		
		r.logger.Info("append entries successfully, set next index and match index", zap.Uint32("peer", result.peerId), zap.Uint64("nextIndex",  r.nextIndex[result.peerId]), zap.Uint64("matchIndex", r.matchIndex[result.peerId]))
	}
	...
	for i := len(logs) - 1; i >= 0; i-- {
		// TODO: (B.9) if there exiss an N such that N > commitIndex, a majority of matchIndex[i] >= N, 
		...
		for peerId, _ := range r.peers {
			...
			if r.matchIndex[peerId] >= r.commitIndex + N  && logs[N-1].GetTerm() == r.currentTerm {
				replicas +=1
			}
	    }

		if replicas >= replicasNeeded {
			r.setCommitIndex(r.commitIndex + N)
			go r.applyLogs(r.applyCh)
			break;
		}
	}
